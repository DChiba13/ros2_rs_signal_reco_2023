...
int main(int argc, char **argv)
{
  ...
  // 赤青黃のHSV表色系での閾値を設定
  MIN_H_RED = g_signal_node->declare_parameter<int>("MIN_H_RED",0);
  MAX_H_RED = g_signal_node->declare_parameter<int>("MAX_H_RED",0);
  MIN_S_RED = g_signal_node->declare_parameter<int>("MIN_S_RED",0);
  MAX_S_RED = g_signal_node->declare_parameter<int>("MAX_S_RED",0);
  MIN_V_RED = g_signal_node->declare_parameter<int>("MIN_V_RED",0);
  MAX_V_RED = g_signal_node->declare_parameter<int>("MAX_V_RED",0);

  MIN_H_GREEN = g_signal_node->declare_parameter<int>("MIN_H_GREEN",0);
  MAX_H_GREEN = g_signal_node->declare_parameter<int>("MAX_H_GREEN",0);
  MIN_S_GREEN = g_signal_node->declare_parameter<int>("MIN_S_GREEN",0);
  MAX_S_GREEN = g_signal_node->declare_parameter<int>("MAX_S_GREEN",0);
  MIN_V_GREEN = g_signal_node->declare_parameter<int>("MIN_V_GREEN",0);
  MAX_V_GREEN = g_signal_node->declare_parameter<int>("MAX_V_GREEN",0);

  MIN_H_YELLOW = g_signal_node->declare_parameter<int>("MIN_H_YELLOW",0);
  MAX_H_YELLOW = g_signal_node->declare_parameter<int>("MAX_H_YELLOW",0);
  MIN_S_YELLOW = g_signal_node->declare_parameter<int>("MIN_S_YELLOW",0);
  MAX_S_YELLOW = g_signal_node->declare_parameter<int>("MAX_S_YELLOW",0);
  MIN_V_YELLOW = g_signal_node->declare_parameter<int>("MIN_V_YELLOW",0);
  MAX_V_YELLOW = g_signal_node->declare_parameter<int>("MAX_V_YELLOW",0);
  ...
  // 画像の上から何％の高さを表示するのか設定　
  IMAGE_ABOVE_RASIO = g_signal_node->declare_parameter<float>("IMAGE_ABOVE_RASIO");
  g_signal_node->get_parameter("IMAGE_ABOVE_RASIO",IMAGE_ABOVE_RASIO);
  
  RCLCPP_INFO(g_signal_node->get_logger(),"%s",IMAGE_ABOVE_RASIO);

  // IMAGE_THRESHフレーム連続で赤,青が認識されると信号とみなす
  IMAGE_THRESH = g_signal_node->declare_parameter<int>("IMAGE_THRESH");
  g_signal_node->get_parameter("IMAGE_THRESH",IMAGE_THRESH);
