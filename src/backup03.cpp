...
rclcpp::Node::SharedPtr g_image_node = nullptr;
rclcpp::Node::SharedPtr g_light_node = nullptr;
...
void cvImageToROSImage(const cv::Mat &src, Image &dst)
{
    ...
    dst.header.stamp = g_image_node->now();
}

void publishImage(Image &src)
{
    auto image_pub = g_image_node->create_publisher<Image>("signal_image",10);
    image_pub->publish(src);
}
...
void onImageSubscribed(const Image::SharedPtr msg)
{
  ...
  {
    auto red_pub = g_light_node->create_publisher<TrafficSignal>("light_msg",10);
    TrafficSignal light_msg;
    light_msg.state = "RedLight";
    RCLCPP_INFO(g_light_node->get_logger(),"%s",light_msg.state.c_str());
    red_pub->publish(light_msg);
    red_cnt = 0;
  }
  if(green_cnt>IMAGE_THRESH)
  {
    auto green_pub = g_light_node->create_publisher<TrafficSignal>("light_msg",10);
    TrafficSignal light_msg;
    light_msg.state = "GreenLight";
    RCLCPP_INFO(g_light_node->get_logger(),"%s",light_msg.state.c_str());
    green_pub->publish(light_msg);
    green_cnt = 0;
  }
...
int main(int argc, char **argv)
{
  ...
  g_image_node = rclcpp::Node::make_shared("camera_image");
  g_light_node = rclcpp::Node::make_shared("light");
  auto sub = g_image_node->create_subscription<Image>("/camera1/image",10,onImageSubscribed);

  rclcpp:spin(g_image_node);
//   cv::destroyAllWindows();
  g_image_node = nullptr;
  g_light_node = nullptr;
  rclcpp::shutdown();
